...
docker-server:
	...
	docker build . -f server.Dockerfile -t $(IMAGE_REPO)/server:$(DOCKER_IMAGE_TAG)
...
docker-server-x:
	...
	docker buildx build . -f server.Dockerfile -t $(IMAGE_REPO)/server:$(DOCKER_IMAGE_TAG) --platform linux/amd64,linux/arm64 --output type=$(DOCKER_BUILDX_OUTPUT)
